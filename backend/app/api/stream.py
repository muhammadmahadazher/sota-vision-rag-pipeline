...
async def process_frames_consumer(websocket: WebSocket, queue: asyncio.Queue):
    ...
    while True:
        try:
            ...
            if frame is None:
                logger.warning(
                    "Decoded frame is None (corrupt/empty). Broadcasting Stream Disconnected status.")
                try:
            ...
            # OPTIMIZATION: Extract once and clean once
            ...
            if rag_engine and rag_engine.qdrant_client:
                if faces and len(faces) > 0 and faces[0].get('embedding') is not None:
                    ...
                    try:
                        ...
                        historical_context = [
                            hit.payload for hit in search_result.points if hit.payload is not None]
                ...
                if rag_engine:
                    try:
                        ...
                        if faces and len(faces) > 0 and faces[0].get('embedding') is not None:
                            ...
                            asyncio.create_task(rag_engine.index_entity(
                                embedding, payload_metadata))
                            logger.info(
                                "Face embedding and narrative indexed in Qdrant.")
                    except Exception as e:
...
async def websocket_stream(websocket: WebSocket):
    ...
    consumer_task = asyncio.create_task(
        process_frames_consumer(websocket, queue))
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.warning(
                    f"Error receiving bytes or sudden disconnect: {e}")
                try:
