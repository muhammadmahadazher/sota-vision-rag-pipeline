...
def test_advanced_pipeline_insightface_retries_on_cpu_when_gpu_initialization_fails():
    gpu_detector = MagicMock()
    cpu_detector = MagicMock()
    detector_factory = MagicMock(side_effect=[gpu_detector, cpu_detector])
    ...
    face_analysis_factory = MagicMock(side_effect=[face_analysis_gpu, face_analysis_cpu])
    ...
    assert call_args_1.kwargs['providers'] == ['CUDAExecutionProvider', 'CPUExecutionProvider']
    face_analysis_gpu.prepare.assert_called_once_with(ctx_id=0, det_size=(640, 640))
    ...
    face_analysis_cpu.prepare.assert_called_once_with(ctx_id=-1, det_size=(640, 640))
