import { cpuRuntime } from "./local-hardware.js";
...
function attachModel(plan) {
  return {
    ...plan,
    modelId: DETECTOR_MODEL.id,
    revision: DETECTOR_MODEL.revision,
    classCount: DETECTOR_MODEL.classCount,
    modelName: DETECTOR_MODEL.name,
  };
}
...
function startModelUpgrade() {
  if (detector || detectorPromise || disposed) return;
  detectorPromise = (async () => {
    publishProgress({
      status: "hardware",
      file: "Starting the accurate RF-DETR CPU/WASM detector",
      progress: 0,
    });
    const cpuPlan = attachModel(cpuRuntime(null));
    ...
    if (!disposed) {
      activateNative(
        `The RF-DETR detector could not start; motion analysis remains active. ${errorDetail(error, "Model loading failed")}`,
      );
    }
    return null;
  });
