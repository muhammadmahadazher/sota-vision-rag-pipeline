...
function errorDetail(error, fallback) {
  ...
  if (error && typeof error === "object") {
    const details = ["name", "message", "code", "reason", "error"]
...
async function getTransformers() {
  if (!transformersPromise) {
    transformersPromise = import(TRANSFORMERS_URL).then((module) => {
      ...
      // Models are bundled with the app. Transformers.js Cache API entries can
      // survive deployments and retain a partial or obsolete model, while the
      // browser HTTP cache already handles these immutable assets safely.
...
function startModelUpgrade(forcedPlan = null) {
  ...
  detectorPromise = (async () => {
    ...
    const discoveredPlan = forcedPlan ?? await selectBrowserHardware(self.navigator);
    const cpuPlan = attachModel(cpuRuntime(
      discoveredPlan.device === "webgpu" ? null : discoveredPlan.fallbackReason,
    ));
    const cpuDetector = await loadDetector(cpuPlan);

    if (disposed) {
      if (cpuDetector?.dispose) await cpuDetector.dispose();
      return null;
    }
    ...
    if (discoveredPlan.device !== "webgpu") return cpuDetector;

    const gpuPlan = attachModel(discoveredPlan);
    publishProgress({
      status: "hardware",
      file: `CPU detector ready; enabling ${gpuPlan.modelName} WebGPU`,
      progress: 100,
    });
    try {
      const gpuDetector = await loadDetector(gpuPlan);
      if (disposed) {
        if (gpuDetector?.dispose) await gpuDetector.dispose();
        return cpuDetector;
      }
      ...
      // A failed WebGPU session can poison a later WASM session in the same
      // worker. Keeping the already-created CPU session makes GPU acceleration
      // a safe progressive enhancement instead of a single point of failure.
      runtimePlan = {
        ...cpuPlan,
        fallbackReason: `GPU upgrade unavailable; CPU detector remains active. ${errorDetail(gpuError, "GPU initialization failed")}`,
      };
      publishRuntime("runtime", "ready");
      publishRuntime("ready", "ready");
      return cpuDetector;
    ...
    if (!disposed) {
      activateNative(
        `The CPU detector could not start; dependency-free motion analysis remains active. ${errorDetail(error, "Model loading failed")}`,
      );
    }
    return null;
  });
