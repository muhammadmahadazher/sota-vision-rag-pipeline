...
describe("bundled browser detector assets", () => {
  it("loads the runtime and model from same-origin public assets", () => {
    const worker = readFileSync(publicFile("local-vision-worker-v4.js"), "utf8");
  ...
  it.each([
    ...
    [["models", "onnx-community", "dfine_s_obj365-ONNX", "onnx", "model_fp16.onnx"], 20_000_000],
  ...
  it("ships the complete 365-class Objects365 label map", () => {
    const config = JSON.parse(
      readFileSync(publicFile("models", "onnx-community", "dfine_s_obj365-ONNX", "config.json"), "utf8"),
    ) as { id2label: Record<string, string> };

    expect(Object.values(config.id2label).filter((label) => label !== "None")).toHaveLength(365);
    expect(Object.values(config.id2label)).toEqual(expect.arrayContaining(["Person", "Chair", "Cell Phone"]));
  });
  it("ships the fast 80-class CPU fallback label map", () => {
    const config = JSON.parse(
